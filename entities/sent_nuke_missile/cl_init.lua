local matGlow = CreateMaterial( "glow", "UnlitGeneric", {
...
function ENT:Draw()
	...
	-- FIX: self.Entity:GetPos/GetForward -> self:GetPos/GetForward
	local Pos = self:GetPos()
	local Ang = self:GetForward()

	if CurrentTime > self.LastEmit + 0.02 then
		-- FIX: self.Entity:GetNWBool -> self:GetNWBool
		self.Armed = self:GetNWBool( "armed", false )
	...
	if SpriteDrawTimeLeft > 0 then
		...
		render.DrawSprite(
			Pos - Ang * 6,
			4800 * SpriteDrawTimeLeft,
			3600 * SpriteDrawTimeLeft,
			Color( 255, 240, 200, 800 * SpriteDrawTimeLeft )
		)
	end

	-- FIX: self.Entity:DrawModel -> self:DrawModel
	self:DrawModel()
end
