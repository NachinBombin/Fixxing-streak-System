...
function drawProgressBar()
	...
	if !lpl:GetNetworkedBool("SupplyCrate_DrawBarBool")  then		
		...
		hook.Remove( "Tick", "Check_Player" )  //  REMOVE CUSTOM HOOK CALLED:	"Check_Player"
...
usermessage.Hook( "CHECK_PLAYER_INPUT", function()  //	IF THE SYSTEM DETECTS THAT A MESSAGE HAS BEEN SENT TO THE USER *AND* THAT MESSAGE IS SPECIFICALLY:	"CHECK_PLAYER_INPUT"  -  DO THE FOLLOWING...


	hook.Add( "Tick", "Check_Player", function()	//	ADD A HOOK NAMED:	"Check_Player"	-	FOR EVERY "TICK" ON THE SYSTEM, DO THE FOLLOWING...


		if LocalPlayer():KeyDown( IN_USE ) == true then  //  CHECK:  IF THE LOCAL PLAYER *HOLDS DOWN* THEIR "USE" KEY, THEN...
	
	
			net.Start( "START_CAPTURING" )  //	BEGIN A (NETWORK) MESSAGE BLOCK:	NAME THE MESSAGE "START_CAPTURING"
		
		
			net.SendToServer()  //	SEND THIS MESSAGE TO THE SERVER
			
			
		else  //  IF THE LOCAL PLAYER IS *NOT HOLDING DOWN* THEIR "USE" KEY ( OR THEY *STOPPED HOLDING DOWN* THE KEY ), THEN...
		
		
			net.Start( "STOP_CAPTURING" )  //	BEGIN A (NETWORK) MESSAGE BLOCK:	NAME THE MESSAGE "STOP_CAPTURING"
	...
	end )  //  COMPLETE THE FUNCTION ( FOR THE NESTED HOOK )


end )  //  COMPLETE THE FUNCTION ( FOR THE PARENT HOOK )
...
function ENT:Draw()
	...
	local tex = Material( "vgui/killstreaks/animated/" .. reward )
