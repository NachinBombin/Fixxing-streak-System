...
function ENT:Initialize()
	-- FIX: GetGlobalInt is removed -> GetConVarNumber
	...
	if self.Yield > 0.13 then
		...
		-- FIX: timer.Simple old vararg API -> anonymous function wrapper
		local ent = self.Entity
		timer.Simple( 0.05, function() PlayPopSound( ent ) end )
	end
end
...
function ENT:Think()
	...
	if not self.HasPlayedSlopdeSnd then
		-- FIX: timer.Simple old vararg API -> anonymous function wrapper
		local vol = 7e5 / CurDist
		timer.Simple( CurDist / 18e3, function() PlaySplodeSound( vol ) end )
		self.HasPlayedSlopdeSnd = true
	end
