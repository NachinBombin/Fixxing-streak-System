...
function ENT:Initialize()
	...
	-- FIX: self.Entity:XXX -> self:XXX throughout
	...
	-- FIX: self.Owner = self.Entity:GetVar('owner') - dead API -> removed; owner set by base
	-- FIX: self.Weapon = self.Entity - deprecated self.Entity alias -> removed
	...
	if self.Yield > 0.13 then
		...
		for _, found in pairs( ents.FindInSphere( self.SplodePos, blastradius ) ) do
				...
				if foundspecs.npc then
					if self.DoDisintegration then
						...
						if foundspecs.humanoid then
							...
							util.Effect( "nuke_disintegrate", ed )
		...
		-- FIX: radiation:SetVar('owner',self.Owner) - dead API -> radiation.Owner
...
function ENT:Think()
	...
	for _, found in pairs( ents.FindInSphere( self.SplodePos, self.SplodeDist ) ) do
		local EntSpecs = self:GetSpecs( found )
		if EntSpecs.valid then
			local entpos = EntSpecs.ent:LocalToWorld( EntSpecs.ent:OBBCenter() )
			if self:LOS( EntSpecs.ent, entpos ) then
				local vecang = entpos - self.SplodePos
				if vecang.z < 0 then vecang.z = 0 end
				vecang:Normalize()
				local DamagePerSecond = self.BaseDamage / ( 4 * math.pi * self.SplodeDist ^ 2 )
				...
				if EntSpecs.movetype ~= 6 or not physobj:IsValid() then
					EntSpecs.ent:SetVelocity( vecang * ( 100 * Damage ) )
				elseif EntSpecs.ragdoll then
					physobj:ApplyForceCenter( vecang * ( 8e4 * Damage ) )
				else
					physobj:ApplyForceOffset(
						vecang * ( 8e4 * Damage ),
						entpos + Vector( math.random( -20, 20 ), math.random( -20, 20 ), math.random( 20, 40 ) )
					)
				end

				local attacker = IsValid( self.Owner ) and self.Owner or self
				util.BlastDamage( self, attacker, entpos - vecang * 64, 384, Damage )
			end
		end
	end
end
...
local HumanModels = {
	"[Ss]oldier", "models/zombie", "models/player/", "models/Humans/",
	"[Pp]olice.mdl", "[Aa]lyx.mdl", "[Bb]arney.mdl", "[Bb]reen.mdl",
	"[Ee]li.mdl", "[Mm]onk.mdl", "[Kk]leiner.mdl", "[Mm]ossman.mdl",
	"[Oo]dessa.mdl", "[Gg]man"
}
...
function ENT:GetSpecs( entity )
	local enttable = {
		ent = entity, player = false, npc = false,
		humanoid = false, ragdoll = false, type = ""
	}
	...
	enttable.valid = ( enttable.movetype == 2 or enttable.movetype == 3
		or enttable.movetype == 5 or enttable.movetype == 6
		or enttable.movetype == 9 )
	...
	if enttable.npc then
		...
		if entmodel then
			for _, model in pairs( HumanModels ) do
...
function ENT:LOS( ent, entpos )
	local trace = { start = self.SplodePos, endpos = entpos }
	local traceRes = util.TraceLine( trace )
