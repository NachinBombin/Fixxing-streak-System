...
function ENT:Think()
	...
	if self.CurSector == nil then
		...
		if sec != self.PrevSector then
			...
			--self.CurSector.MidPoint.Prop:SetColor(Color(0,255,0,0)) ---------------------
	...
	elseif self.IsInSector && self.CurSector != nil then
		...
		if self.SectorDelay < CurTime() then
			--self.CurSector.MidPoint.Prop:SetColor(Color(255,255,255,0)) ---------------
...
function ENT:SetupSectors()
	...
	local x1, x2, y1, y2 = pos.x + 5120,pos.x - 5120,pos.y + 5120,pos.y - 5120	--self.MapBounds.xPos, self.MapBounds.xNeg, self.MapBounds.yPos, self.MapBounds.yNeg;
	local tX, tY = 0, 0;
	local bool = true;
	while bool do
		tX, tY = 0, 0;
		x1 = pos.x + 5120--self.MapBounds.xPos;
		while x1 >= x2 do
