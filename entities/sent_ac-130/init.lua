...
function ENT:PhysicsUpdate()
	...
	if self.StayAlive then
		...
		if self.Owner:Alive() == false then		//	CHECK:	IF AT ANY POINT THE OWNER OF THE AC-130 IS *NOT* ALIVE (KILLED IN ACTION), THEN...
		
		
			self:RemoveAC130()	//	GRACEFULLY REMOVE THE AC-130
...
function ENT:Initialize()	
	...
	self:REQUEST_LOCKHEED()  //	AFTER THE AC-130 HAS BEEN SUCCESSFULLY INITIALIZED, RUN CUSTOM FUNCTION EXPLAINED BELOW
	
	
end


function ENT:REQUEST_LOCKHEED()  //	CREATE A GLOBAL FUNCTION CALLED:	"REQUEST_LOCKHEED"
	...
	if GetConVar( "MW2_TEAMS_ENABLED" ):GetInt() != 0 then	//	CHECK:	IF TEAMS *ARE ENABLED*, THEN...
		...
		for Key, Value in pairs( Players ) do		//	CREATE A LOOP IN ORDER TO ITERATE THROUGH EACH PLAYER FOUND  -  FOR EACH PLAYER FOUND, DO THE FOLLOWING...
			...
			if Value:Team() == self.Owner:Team() then	//	IF THE CURRENT PLAYER BEING LOOKED AT IS ON THE *SAME TEAM* AS THE OWNER OF THE AC-130, THEN...
			
			
				umsg.Start( "MW2_LOCKHEED_FRIENDLY", Value );  //  CREATE A MESSAGE (EVENT) CALLED:	"MW2_LOCKHEED_FRIENDLY"	-	SEND TO THE PLAYER CURRENTLY BEING PROCESSED
			
			
				umsg.End();  //  TELL THE SYSTEM THAT ALL MESSAGES HAVE BEEN DEFINED

		
			elseif Value:Team() != self.Owner:Team() then	//	IF THE CURRENT PLAYER BEING LOOKED AT IS *NOT* ON THE SAME TEAM AS THE OWNER OF THE AC-130, THEN...
		

				umsg.Start( "MW2_LOCKHEED_ENEMY", Value );  //  CREATE A MESSAGE (EVENT) CALLED:	"MW2_LOCKHEED_ENEMY"	-	SEND TO THE PLAYER CURRENTLY BEING PROCESSED
				...
				print( "[ AC-130 BROADCAST FAILED ] - TRIED TO SEND MESSAGE TO: ", Value )		//	PRINT ERROR MESSAGE TO SERVER CONSOLE
		...
		for Key, Value in pairs( Players ) do		//	CREATE A LOOP IN ORDER TO ITERATE THROUGH EACH PLAYER FOUND  -  FOR EACH PLAYER FOUND, DO THE FOLLOWING...
			...
			if Value == self.Owner then  //  IF THE PLAYER CURRENTLY BEING LOOKED AT *IS THE OWNER* OF THE AC-130, THEN...
			...
			elseif Value != self.Owner then		//	IF THE PLAYER CURRENTLY BEING LOOKED AT IS *NOT THE OWNER* OF THE AC-130, THEN...
...
function ENT:FireMissile(target, weaponType)
	...
	self.missile:Spawn()
	--self.Owner:EmitSound(missileSound)
...
function ENT:RemoveAC130()
	if timer.Exists("TimerStop105mmRel") then
		timer.Destroy("TimerStop105mmRel")
	end
	if timer.Exists("TimerStop40mmRel") then
		timer.Destroy("TimerStop40mmRel")
	end
	if timer.Exists("TimerStop25mmRel") then
